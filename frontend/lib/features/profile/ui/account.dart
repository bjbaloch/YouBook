/// Account UI - User account page
import 'dart:async';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:final_year_project/features/profile/logic/profile_logic.dart';
...
import 'package:final_year_project/profile/change_email/change_email.dart';
import 'package:final_year_project/profile/change_password/change_password.dart';
...
class _AccountPageState extends State<AccountPage> {
  ...
  late final ProfileLogic _logic;
  late final SupabaseClient _sb;

  @override
  void initState() {
    super.initState();
    _logic = ProfileLogic(context: context);
  ...
  Widget _profileHeaderCard(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    final profile = _profileData ?? ProfileData.empty();
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _roField(context, icon: Icons.person, text: profile.fullName, isPrimaryOn: true),
          const SizedBox(height: 10),
          _roField(context, icon: Icons.badge, text: profile.cnic ?? 'CNIC', isPrimaryOn: true),
          const SizedBox(height: 10),
          _roField(context, icon: Icons.phone, text: profile.phone ?? 'Phone number', isPrimaryOn: true),
          const SizedBox(height: 10),
          _roField(context, icon: Icons.email_outlined, text: profile.email, isPrimaryOn: true),
          const SizedBox(height: 10),
          _roField(context, icon: Icons.location_on, text: profile.address ?? 'Address', isPrimaryOn: true),
          const SizedBox(height: 10),
          _roField(context, icon: Icons.location_city, text: profile.city ?? 'City', isPrimaryOn: true),
          const SizedBox(height: 10),
          _roField(context, icon: Icons.map_outlined, text: profile.stateProvince ?? 'State/Province', isPrimaryOn: true),
          const SizedBox(height: 10),
          _roField(context, icon: Icons.flag_outlined, text: profile.country ?? 'Country', isPrimaryOn: true),
          const SizedBox(height: 10),
          Align(
